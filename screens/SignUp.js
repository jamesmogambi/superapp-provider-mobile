...
const SignUp = () => {
  ...
  // Step 1: create the account and send an email verification code.
  ...
  // Step 2: verify the emailed code and activate the session.
  const onVerify = useCallback(async () => {
    ...
    try {
      ...
      if (attempt.status === "complete") {
        // Session ready -> RootStack renders HomeStack (redirect to Home).
        await setActive({ session: attempt.createdSessionId });
      } else {
        console.warn("Sign-up incomplete, status:", attempt.status);
  ...
  }, [isLoaded, loading, code, signUp, setActive]);
        ...
        {!pendingVerification ? (
          <>
            <View className="space-y-2 mb-8">
              <Text className="text-2xl text-neutral-800">Get started with</Text>
          ...
          <>
            <View className="space-y-2 mb-8">
              <Text className="text-2xl text-neutral-800">Verify your email</Text>
